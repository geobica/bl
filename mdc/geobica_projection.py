...
def sc_map_continuous(zp,z,beta,p):
    ...
    quadrature_spot_check(zp,z,beta,p,wp)
    return wp

def sc_map_continuous_adaptive(zp,z,beta,p):
    ...
    for i in range(wp.shape[0]):
        if zp[i]==0:
            continue
        direction = zp[i]/np.absolute(zp[i])
        wreal = integrate.quad(lambda zeta : np.real(f_prime_W(direction*zeta)*direction), 0, np.absolute(zp[i]))[0]
        wimag = integrate.quad(lambda zeta : np.imag(f_prime_W(direction*zeta)*direction), 0, np.absolute(zp[i]))[0]
        wp[i] = (wreal+1j*wimag)
    return wp

quad_check_calls = 0

def quadrature_spot_check(zp,z,beta,p,wp):
    global quad_check_calls
    if not QUAD_CHECK:
        return
    quad_check_calls += 1
    if quad_check_calls>4 and quad_check_calls%200!=0:
        return
    reference = sc_map_continuous_adaptive(zp,z,beta,p)
    scale = np.maximum(np.absolute(reference),1e-300)
    worst = np.amax(np.absolute(wp.reshape(-1)-reference)/scale)
    print(f'quadrature spot check {quad_check_calls}: worst relative {worst:.3g}')
...
def explore_polygon(poly_coords,preceeder,centers_pickled=0,inverted_buffer_metric=False,nesolve_trace='',crpfun_workers=0):
    poly_string = 'A = ['
    ...
    if not os.path.isfile(f'pickle/matlab_saves/{preceeder}sc.mat'):
        ...
        _sc_start = time.time()
        _oc().feval('sc_toolbox_octave/layer_0.m',_oc().pull('A'),_oc().pull('A_D'),_oc().pull('outfile'),nesolve_trace,crpfun_workers)
...
def run_pipeline(name,centers_pickled=0):
    global beta_raw
    boundary_stride = 10
...
if __name__=='__main__':
    ...
    if len(sys.argv)>1:
        ...
        parser.add_argument('centers_pickled', nargs='?', type=int, default=0)
        args = parser.parse_args()

        run_pipeline(args.name,args.centers_pickled)
    else:
