...
def pick_center_interactively(name,input_path):
    # Menu that shows you a map to let you select the center point you want to use
    # Might remove this later, it's a bit excessive, and it might be better to just guess a center
    import curses
    import shapely
    ...
    def run(stdscr):
        ...
        lon,lat = 0,0
        cached_size = None
        land = map_rows = map_cols = None
        while True:
            max_y,max_x = stdscr.getmaxyx()
            if (max_y,max_x)!=cached_size:
                header_lines = 2
                map_cols = max(20,max_x-1)
                map_rows = max(8,min(max_y-header_lines-1,round(map_cols*0.25)))
                lon_bins = -180+(np.arange(map_cols)+0.5)/map_cols*360
                lat_bins = 90-(np.arange(map_rows)+0.5)/map_rows*180
                grid_lon,grid_lat = np.meshgrid(lon_bins,lat_bins)
                land = shapely.contains_xy(region,grid_lon.ravel(),grid_lat.ravel()).reshape(grid_lon.shape)
                cached_size = (max_y,max_x)

            x_col = min(max(int((lon+180)/360*map_cols),0),map_cols-1)
            x_row = min(max(int((90-lat)/180*map_rows),0),map_rows-1)
            alon,alat = antipode(lon,lat)
            o_col = min(max(int((alon+180)/360*map_cols),0),map_cols-1)
            o_row = min(max(int((90-alat)/180*map_rows),0),map_rows-1)

            x_in = bool(shapely.contains_xy(region,float(lon),float(lat)))
            o_in = bool(shapely.contains_xy(region,float(alon),float(alat)))
            valid = x_in and not o_in
            attr = (curses.color_pair(1) if colors_ok else curses.A_REVERSE) if valid else curses.A_NORMAL

            stdscr.erase()
            status = 'valid: X in region, O at sea' if valid else 'invalid: put X in the shape, keep O out of it'
            stdscr.addstr(0,0,f'lon {lon:+4d}  lat {lat:+3d}   {status}'[:max_x-1],attr)
            for r in range(map_rows):
                row_chars = []
                for c in range(map_cols):
                    if r==x_row and c==x_col:
                        row_chars.append('X')
                    elif r==o_row and c==o_col:
                        row_chars.append('O')
                    else:
                        row_chars.append('█' if land[r,c] else ' ')
                try:
                    stdscr.addstr(header_lines+r,0,''.join(row_chars),attr)
                except curses.error:
                    pass
            try:
                stdscr.addstr(min(header_lines+map_rows,max_y-1),0,'arrows/wasd/hjkl move, enter confirms, q/esc cancels'[:max_x-1])
            except curses.error:
                pass
            stdscr.refresh()
            ...
            if key in (curses.KEY_UP,ord('k'),ord('w')):
                ...
                lon = lon-1 if lon>-180 else 179
            elif key in (curses.KEY_RIGHT,ord('l'),ord('d')):
                lon = lon+1 if lon<179 else -180
...
def run(name,lon_in=None,lat_in=None,input_path='input_sample',pickle_path='pickle/bubble_wrap'):
    if lon_in is None or lat_in is None:
        lon_in,lat_in = pick_center_interactively(name,input_path)
...
if __name__=='__main__':
    ...
    parser.add_argument('lon', nargs='?', type=int, default=None,
        help='longitude of a point inside the region; omit (with lat) to pick interactively')
    parser.add_argument('lat', nargs='?', type=int, default=None,
        help='latitude of a point inside the region; omit (with lon) to pick interactively')
    args = parser.parse_args()

    run(args.name,args.lon,args.lat)