...
def run(name,vector_file=None,outline_out=None,projection_out=None,mesh_out=None,extra_outputs=True,graticule_out=None,graticule_step_deg=None,angle_outlier_deg=None):
    out_name = name
    ...
    (grid_sample_w,mapped_sample_points,loaded_pickle,opposing_point,
        cell_count,boundary_count,outline_point_location,_rotation_phase) = _load_rotated_mesh(name)
    ...
    if extra_outputs:
        multipolys = read_in_file(vector_file,equi_to_stereo,clip_poly=clip_poly,clip_forward=_clip_forward,clip_inverse=_clip_inverse,progress_label='Projecting to stereographic (original)')
        multi_to_file(f'maps_projected/original_{out_name}.shp',multipolys)

    multipolys = read_in_file(vector_file,reproj,clip_poly=clip_poly,clip_forward=_clip_forward,clip_inverse=_clip_inverse,progress_label='Projecting to final position')
    ...
    angle_errors,length_errors,angle_outliers = measure_distortion(vector_file,clip_poly,progress_label='Measuring distortion')
...
def run_raster(name,raster_path,out_png=None,resolution=1200):
...
def run_distortion(name,out_shp=None):
...
if __name__=='__main__':
    ...
    parser.add_argument('vector_file', help='a vector shapefile to project')
    args = parser.parse_args()

    run(args.name,args.vector_file)
