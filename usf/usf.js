...
function remake_radio(){
    let num_stars = parseInt(document.getElementById("star_count").value);
    console.log(data[num_stars].slice());
    let default_method = data[num_stars].slice()[2];
    console.log(default_method);
    if(default_method==0){
        document.getElementById('rectangular').click();
        document.getElementById('rect_w').value = data[num_stars].slice()[0];
        document.getElementById('rect_h').value = data[num_stars].slice()[1];
        document.getElementById('rect_alter').value = data[num_stars].slice()[3];

        document.getElementById('diagA_w').value = data[num_stars].slice()[0]*2;
        document.getElementById('diagA_h').value = data[num_stars].slice()[1];
        document.getElementById('diagA_alter').value = data[num_stars].slice()[3]*2;

        document.getElementById('diagB_w').value = data[num_stars].slice()[0]*2;
        document.getElementById('diagB_h').value = data[num_stars].slice()[1];
        document.getElementById('diagB_alter').value = data[num_stars].slice()[3]*2;
    }else if(default_method==1){
        document.getElementById('diagonal_A').click();
        document.getElementById('diagA_w').value = data[num_stars].slice()[0];
        document.getElementById('diagA_h').value = data[num_stars].slice()[1];
        document.getElementById('diagA_alter').value = data[num_stars].slice()[3];

        document.getElementById('diagB_w').value = data[num_stars].slice()[0];
        document.getElementById('diagB_h').value = data[num_stars].slice()[1];
        document.getElementById('diagB_alter').value = data[num_stars].slice()[3];

        if(Math.floor((1+data[num_stars].slice()[0])/2)<data[num_stars].slice()[1]){
            document.getElementById('rect_w').value = data[num_stars].slice()[1];
            document.getElementById('rect_h').value = Math.floor((1+data[num_stars].slice()[0])/2);
        }else{
            document.getElementById('rect_w').value = Math.floor((1+data[num_stars].slice()[0])/2);
            document.getElementById('rect_h').value = data[num_stars].slice()[1];
        }
        document.getElementById('rect_alter').value = Math.floor((1+data[num_stars].slice()[0])/2)*data[num_stars].slice()[1]-num_stars;
    }else{
        document.getElementById('diagonal_B').click();
        document.getElementById('diagB_w').value = data[num_stars].slice()[0];
        document.getElementById('diagB_h').value = data[num_stars].slice()[1];
        document.getElementById('diagB_alter').value = data[num_stars].slice()[3];
...
function make_flag(x,y,method,alter,str_cou,canton_width,canton_height,aspect_ratio,first_stripe_red,star_points,star_points_2,use_radio=true){
    ...
    if(use_radio&&method!=3){
        ...
        if(selected_radio_button!=null){
            ...
            if(selected_radio_button.id=="rectangular"){
                method=0;
            }else if(selected_radio_button.id=="diagonal_A"){
                method=1;
            }else if(selected_radio_button.id=="diagonal_B"){
                method=2;
            }
        }
        // method = parseInt(document.getElementById('rect_w').value);\
        if(method==0){
            x = parseInt(document.getElementById('rect_w').value);
            y = parseInt(document.getElementById('rect_h').value);
            alter = parseInt(document.getElementById('rect_alter').value);
        }else if(method==1){
            x = parseInt(document.getElementById('diagA_w').value);
            y = parseInt(document.getElementById('diagA_h').value);
            alter = parseInt(document.getElementById('diagA_alter').value);
        }else if(method==2){
            x = parseInt(document.getElementById('diagB_w').value);
            y = parseInt(document.getElementById('diagB_h').value);
            alter = parseInt(document.getElementById('diagB_alter').value);
        }
    }
...
export {modify_flag, download_flag, download_flag_png, remake_radio};